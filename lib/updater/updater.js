...
- Edit {{versions}}.json and add base URI for old versions
- Check manifest
- Sync new version's ID to ./version file
- Update in local first
*/
    ...
    async loadVersionsList() {
        ...
        try {
            ...
            for (const i of newData.versions) {
                ...
                ver.append(label);
                el.append(ver);
                if (i.id === currentVersion) (label.append(" (current)"));
                if (i.locked === true) (checkbox.remove(), label.append(" [Locked]"), (label.style.textDecoration = 'line-through'));
                const cont = document.createElement('div');
                cont.className = 'update-card-content';
...
const requestIdleCallback = globalThis.requestIdleCallback || function (callback, options) {
...
async function commonUpdater_worker() {
    ...
    const retry_button = document.createElement('a');
    retry_button.href = 'javascript:void(0)';
    retry_button.append(tr('p:retry'));
    retry_button.hidden = true;
    retry_button.style.marginRight = '1em';
    retry_button.onclick = () => location.reload();
    cancel_button_container.append(retry_button);
    ...
    const makeCancelable = (fn) => {
        ...
        cancel_button_container.hidden = cancel_button.hidden = false;
    };
    ...
    switch (prog) {
        case '1': {
            ...
            try {
                ...
                if (json.versions[targetIndex].locked) throw new Error('tried to install a locked version');
                await delay(500);
                {
                    let filesToDl = {};
        ...
        case '2': {
            ...
            try {
                ...
                for (const i of keys) {
                    ...
                    if (!resp.ok) throw `HTTP Error ${resp.status}: ${resp.statusText}`;
                    cache.put(new Request(base + (i === 'index.html' ? '' : i)), await commonUpdater_fixMimeTypes(data.filesToDl[i], resp));
...
export async function commonUpdater_fixMimeTypes(req, resp) {
    const url = (req instanceof Request) ? req.url : req;
    const ext = url.substring(url.lastIndexOf('.'));
    const headers = {
        'content-type': mimeTypes[ext] || resp.headers.get('content-type'),
        'cache-control': 'no-cache',
    };
    const requiredHeaders = ['content-length', 'etag', 'date', 'expires',];
    for (const i of requiredHeaders) {
        headers[i] = resp.headers.get(i);
    }
    const blob = await resp.blob();
    const body = (mimeTypes[ext]) ? (new Blob([blob], { type: mimeTypes[ext] })) : blob;
