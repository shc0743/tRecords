...
<div style="margin-top: 10px" class="infobox" id="changedTips" hidden>${tr('reloadReq')} <a href="javascript:" id="reloadNow">${tr('reloadNow')}</a></div>
...
<dialog id=moreExts>
    ...
    <fieldset id=installFromUrl>
        ...
        <input type=url id=url style="box-sizing: border-box; width: 100%;" />
        <div style="text-align: right; margin-top: 0.5em;"><button type=button id=installUrl>${tr('install')}</button></div>
    </fieldset>
...
export class ExtensionManager {
    ...
    async install(url) {
        ...
        const resp = await fetch(urlScript);
